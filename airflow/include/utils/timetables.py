...
def transform_fchg_events_to_dwh() -> Dict[str, int]:
    batch_id = db_utils._get_latest_batch_id("psa.timetables_fchg_raw")
    if batch_id is None:
        return {"rows_inserted": 0, "batch_id": None}
    if db_utils.is_batch_processed("db_timetables_fchg_transform", batch_id):
        return {"rows_inserted": 0, "batch_id": batch_id, "skipped": 1}
    process_id = db_utils.log_process_start(process_name="db_timetables_fchg_transform", batch_id=batch_id)
    try:
        inserted = db_utils.insert_timetables_fchg_events_to_dwh(batch_id=batch_id)
        db_utils.log_process_end(process_id=process_id, status="success", message=f"rows_inserted={inserted}")
